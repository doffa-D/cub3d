...
/*   Updated: 2023/07/20 15:33:48 by nouakhro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_horz(t_struct *cub, float ray_angle, int strip_id, t_steps *vars)
{
	...
	{
		if (open_close_horz_dor(cub, strip_id, vars))
			break ;
		else if (hit_horz_wall(cub, vars))
			break ;
		vars->start_x += vars->x;
		vars->start_y += vars->y;
	}
...
void	check_vert(t_struct *cub, int strip_id, t_steps *vars)
{
	...
	{
		if (open_close_vert_dor(cub, strip_id, vars))
			break ;
		else if (hit_vert_wall(cub, vars))
			break ;
		vars->start_x += vars->x;
		vars->start_y += vars->y;
	}
...
void	cast_ray(t_struct *cub, float ray_angle, int strip_id)
{
	...
	if (vars.vert_distance < vars.horz_distance)
	{
		...
		cub->rays[strip_id].wall_hit_content = vars.vert_content;
	}
	else
	{
		...
		cub->rays[strip_id].wall_hit_content = vars.horz_content;
	}
}
